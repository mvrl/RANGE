...
def get_args():
    ...
    parser.add_argument('--region_type', type=str, default='ECO_BIOME_') 
...
if __name__ == '__main__':
    ...
    if args.pcme_sapclip:
        model = SAPCLIP_PCME.load_from_checkpoint(args.ckpt_path).to(device)
    else:
        model = SAPCLIP.load_from_checkpoint(args.ckpt_path).to(device)
    ...
    if model.scale_encoding=='learnable':
        map_scale = {1:torch.tensor(0).cuda(),
    ...
    elif model.scale_encoding=='onehot':
    ...
    clf_sapclip.fit(xtest_sapclip, ytest)

    acc = clf_sapclip.score(xtrain_sapclip, ytrain)
    print(f'BIOME_ACC: {acc}')
    ...
    acc = clf_sapclip.score(xtest_sapclip, ytest)
    print(f'BIOME_ACC: {acc}')
