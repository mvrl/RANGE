...
def load_dataset(params, eval_split, train_remove_invalid=True, eval_remove_invalid=True,
                 load_cnn_predictions=True, load_cnn_features=False,
...
def load_inat_data(ip_dir, loc_file_name, ann_file_name, remove_empty=False, load_img = False):
    ...
    if load_img:
        return np.array(locs).astype(np.float32), np.array(classes).astype(int), \
               np.array(users).astype(int), np.array(dates).astype(np.float32), np.array(keep_inds), np.array(imgs)

    else:
        return np.array(locs).astype(np.float32), np.array(classes).astype(int), \
               np.array(users).astype(int), np.array(dates).astype(np.float32), np.array(keep_inds)
...
def load_yfcc_data(data_dir, ann_file_name, split_name):
    ...
    params = {'dataset':'inat_2018', 'load_img':False,
          'cnn_pred_type':'full', 'inat2018_resolution':'high_res'}
        
    da = pd.read_csv(data_dir + ann_file_name)
    locs = da[da['split'] == split_name][['lon', 'lat']].values.astype(np.float32)
    classes = da[da['split'] == split_name]['class'].valuesdataset
...
if __name__ == '__main__':
    ...
    dataset = load_dataset(params, 'val')
